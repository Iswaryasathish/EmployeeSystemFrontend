...
function UpdateEmployee() {
  ...
  const fetchEmployee = async () => {
    try {
      const res = await axios.get(`https://employeeprofilebackend-1.onrender.com/api/employees/${employeeId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/employees/${employeeId}`, updatedEmployee);
